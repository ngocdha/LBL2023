...
for trial = 1:4
    ...
    for ih=1:nh
        ...
        [evecs, evals] = eig(H_tot);%use this if the matrix is NOT SPARSE
    ...
    %disp(mx_list)
    mx_master = [mx_master,mx_list];
end

% === 6. Plots ===
figure;

plot(h_str, mx_master(1:200), '-','LineWidth', 2);% Line with circles
hold on;
plot(h_str, mx_master(201:400), '-', 'LineWidth', 2);
hold on;
plot(h_str, mx_master(401:600), '-','LineWidth', 2);
hold on;
plot(h_str, mx_master(601:800), '-','LineWidth', 2);
legend('J=-1', 'J=0', 'J=1', 'J=2')
hold off;
xlabel('External magnetic filed');
ylabel('Average magnetization');
str = sprintf('Magnetic Order, |Λ|=%d', n);
